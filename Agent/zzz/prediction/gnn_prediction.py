...
class Prediction_Model_Training():
    ...
    def __init__(self):
        ...
        for i in range(self.heads_num):
            predition_model = GNN_Prediction_Model(self.history_frame * 5, self.future_frame * 2, self.obs_scale, Use_Gaussion_Output).to(self.device)
            predition_model.apply(self.weight_init)
            self.ensemble_models.append(predition_model)
            self.ensemble_optimizer.append(torch.optim.Adam(predition_model.parameters(), lr = 0.001))
        ...
        if len(self.data) > 0:
            ...
            for i in range(self.heads_num):
                ...
                if Use_Gaussion_Output:
                    predict_action, sigma = self.ensemble_models[i](history_data)
            ...
            # closed loop test
            # self.predict_future_paths(one_trajectory[0])
            # self.predict_future_paths(one_trajectory[1])
            # self.predict_future_paths(one_trajectory[1])
            # self.predict_future_paths(one_trajectory[1])
            # paths_of_all_models = self.predict_future_paths(one_trajectory[2])
            # print("paths_of_all_models.x",paths_of_all_models[0].x[-1])
            # print("paths_of_all_models.y",paths_of_all_models[0].y[-1])
            # print("realPath.x",one_trajectory[32][1][0])
            # print("realPath.y",one_trajectory[32][1][1])
            
            self.trained_data.append(self.data[0])
            self.data.pop(0)
    ...
    def predict_future_paths(self, obs):
        ...
        if len(self.infer_obs_list) >= self.history_frame:
            ...
            for i in range(self.heads_num):
                ...
                if Use_Gaussion_Output:
                    ...
                    predict_action = self.ensemble_models[i](history_data).cpu().detach().numpy()
    ...
    def load_prediction_model(self, load_step):
        try:
            for i in range(self.heads_num):
                ...
                torch.load('save_model/ensemble_models_%s_%s.pt' % (load_step, i))
                )
...
class Prediction():
    def __init__(self, considered_obs_num, maxt, dt, robot_radius, radius_speed_ratio, move_gap):
        ...
        self.gnn_predictin_model.load_prediction_model(30000)
    ...
    def update_prediction(self, dynamic_map):
        ...
        self.predict_paths = self.gnn_predictin_model.predict_future_paths(dynamic_map.real_time_obs)
